...
//#include "hdc1080.h" //SDA on Port 21, SCL on Port 22
#include <Wire.h>
...
#include "lorakeys.h" // Keys fuer weitere Sensoren.
...
static uint8_t mydata[] = {0,0,0,0,0,0}; //Little Endian, ppm CO2, temp in 1/100 Grad Kelvin, humidity in 0.01%
...
void loop() {
  ...
  if (nextCall<millis()) {
    ...
    Serial.printf("Status=%d, ppm=%d, CO2temp=%d, temp=%f, hum=%f\r\n", 
      co2sensor.status, co2sensor.ppmCO2, co2sensor.temp,
      temp, humidity
    );
    ...
    if (co2sensor.status) {
      myStepper.gotoPPM(co2sensor.ppmCO2);
    }
    mydata[0] = co2sensor.ppmCO2 & 255;
    mydata[1] = co2sensor.ppmCO2 >> 8;
    unsigned int tmp = (unsigned int)((temp+273.0)*100.0);
    mydata[2] = tmp & 255;
    mydata[3] = tmp >> 8;
    tmp = (unsigned int)(humidity*100);
    mydata[4] = tmp & 255;
    mydata[5] = tmp >> 8;

    //mystrip.SetPixelColor(0,colorGamma.Correct(RgbColor(mydata[0],mydata[2],mydata[4])));
    //mystrip.Show();
